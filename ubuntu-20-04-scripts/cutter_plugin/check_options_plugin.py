...
#         disasm_callee = cutter.cmd("pdr @ $F").strip()
#         print(disasm_callee)
...
#         disasm_caller = cutter.cmd("pdr @ " + str(callee_addr))
#         print(disasm_caller)
...
class CheckOptionsCutterPlugin(cutter.CutterPlugin):
    name = "check_options plugin"
    description = "check options plugin"
    version = "0.1"
    author = "flo"
    ...
    def setupInterface(self, main):
        action = QAction("Check options Plugin", main)
        action.setCheckable(True)
        widget = CheckOptionsDockWidget(main, action)
...
def create_cutter_plugin():
    return CheckOptionsCutterPlugin()