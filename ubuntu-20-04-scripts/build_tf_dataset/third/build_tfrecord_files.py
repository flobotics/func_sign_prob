...
def main():
    ...
    ## if we got a vocab file, we use it
    if config['vocab_file']:
        vocab_word_list = get_vocab(config)
    else:
        print(f'No vocab file as argument')
    ...
    pickle_files_int = get_all_pickle_filenames(config['label_ints_dir'])
    for file in pickle_files_int:
        dis_list.clear()
        ret_list.clear()
        
        cont = get_pickle_file_content(config['label_ints_dir'] + file)
        
        if (ds_counter+1) >= nr_of_pickle_files:
            print(f'From file >{config["label_ints_dir"] + file}< nr >{ds_counter+1}/{nr_of_pickle_files}<', end='\n')
        else:
            print(f'From file >{config["label_ints_dir"] + file}< nr >{ds_counter+1}/{nr_of_pickle_files}<', end='\r')
